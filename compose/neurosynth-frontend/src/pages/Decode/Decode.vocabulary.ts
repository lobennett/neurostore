...
export const COGNITIVE_ATLAS_CONCEPTS: ICognitiveConcept[] = Array.from(
    new Map(
        (snapshot as CognitiveAtlasSnapshotRecord[])
            .filter((record) => typeof record.id === 'string' && record.id.startsWith('trm_') && typeof record.name === 'string')
            .map((record) => [record.id as string, { id: record.id as string, label: record.name as string, vocabulary: 'Cognitive Atlas' as const }])
    ).values()
