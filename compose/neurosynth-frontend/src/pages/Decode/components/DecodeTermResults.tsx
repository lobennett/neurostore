    ...
    return (
        <Box>
            <Typography variant="body2" color="text.secondary" sx={{ marginBottom: 2 }}>
                Terms whose meta-analytic maps look most like yours. Select one to compare the two maps side by side.
            </Typography>
            <TableContainer>
                <Table size="small">
                    <TableHead>
                    ...
                    <TableBody>
                        {terms.map(({ term, correlation }) => {
                            const direction = correlation === 0 ? 'neutral' : correlation < 0 ? 'negative' : 'positive';

                            return (
                                <TableRow key={term} hover selected={term === selectedTerm}>
                                    <TableCell>
                                        <Button
                                            variant="text"
                                            ...
                                            sx={{ textTransform: 'none', overflowWrap: 'anywhere', minWidth: 0 }}
                                        >
                                            {term}
                                        </Button>
                                    </TableCell>
                                    <TableCell>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            <Box
                                                aria-label={`${term}: ${direction} correlation ${correlation.toFixed(3)}`}
